...
function openYouAreHereMessage() {
  var youAreHereMessage = document.getElementById('youAreHereMessage');
  youAreHereMessage.style.display = 'block';
}

function closeYouAreHereMessage() {
  var youAreHereMessage = document.getElementById('youAreHereMessage');
  youAreHereMessage.style.display = 'none';
}
...
document.addEventListener('keydown', function (event) {
  ...
  if (key1Pressed && key2Pressed) {
    openYouAreHereMessage();
  }
});

document.addEventListener('keyup', function (event) {
  if (event.key === 'Escape') {
    closeYouAreHereMessage();
    closeMsgBox(); // Закрывайте оба сообщения при нажатии Escape
  }
});
